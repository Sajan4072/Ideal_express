...
const Header = () => {
  ...
  return (
    <div className={`py-2 font-roboto  bg-white ${fixnav ? 'fixed shadow-md md:py-0 md:bg-white text-black z-50 animate-fadeInDown  ' : 'md:py-0 absolute md:bg-black text-white md:bg-opacity-30'}  border-b border-transparent`}>
        ...
        <div className='col-span-6 md:col-span-8 justify-self-end flex justify-center'>
          ...
          <div className={`${fixnav ? "" : "md:mt-6"} `}>
            <div className={` hidden  ${fixnav ? 'hidden' : 'md:flex'}  flex-row-reverse gap-x-6`}>
              <div>
                <a href="" className='p-3 transition ease-in-out delay-150 hover:-translate-y-3 hover:scale-110 rounded-md hover:bg-orange-500 hover:text-white bg-white text-black text-sm font-bold'><i className="fi fi-br-home"></i><span className='ml-2 '>Branch Login</span></a>
              </div>
            ...
            <div className={`md:flex hidden ${fixnav ? '  mt-8 items-center' : 'mt-8 items-end'} flex justify-center  relative  gap-x-11`}>
              {MenuData.map((data, index) => {
                return (
                  <Link href={data.url}>
                    <div onClick={() => activeNav(index)} className={`${fixnav ? 'pb-3' : 'pb-3'} ${nav === index && 'active-nav'} relative text-base font-bold items-end before:hover:wiggle before:contents['']  before:absolute before:h-[0.155rem] before:bottom-0 before:w-[100%] ${fixnav ? "before:hover:bg-black" : "before:hover:bg-orange-500"} cursor-pointer before:hover:transition before:hover:duration-[0.8s] before:hover:ease-ease  before:hover:delay-[0s] before:hover:animate-wiggle hover:text-orange-500`}>{data.name}</div>
                  </Link>
                )
              })
              }
